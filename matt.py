...
class CliGameUi(object):

    def get_my_marker(self):
        return raw_input()

    def get_board(self):
        return [list(raw_input()) for _ in range(3)]
...
class Game(object):
    ...
    def get_move(self):
        ...
        if self.board[1][1] == '_':            
            if self.board[0][0] == self.opponent and (self.board[0] + self.board[1] + self.board[2]).count(self.my_marker) == 0:
                return (1, 1)
    ...
    def get_diagonal_winning_move(self, marker):
        ...
        if self.board[1][1] == '_':
            if (self.board[2][0], self.board[0][2]) == two_in_a_row or (self.board[0][0], self.board[2][2]) == two_in_a_row:
                return 1, 1
